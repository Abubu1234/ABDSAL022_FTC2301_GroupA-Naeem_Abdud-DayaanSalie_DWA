...
// @ts-ignore
domElements.lists.dataListItems.appendChild(starting);
...
if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
    // @ts-ignore
    domElements.settings.value = 'night'
...
// @ts-ignore
domElements.search.dataHeaderSearch.addEventListener('click', () => {
...
// @ts-ignore
domElements.settings.dataHeaderSettings.addEventListener('click', () => {
...
domElements.search.dataSearchForm.addEventListener('submit', (event) => {
    ...
    for (const {id, image, title, author } of result.slice(0, BOOKS_PER_PAGE)) {
        const element = document.createElement('button');
                // @ts-ignore
...
domElements.list.dataListButton.addEventListener('click', () => {
    ...
    for (const { id, image, title, author } of matches.slice(page * BOOKS_PER_PAGE, (page + 1) * BOOKS_PER_PAGE)) {
        const element = document.createElement('button');
                // @ts-ignore
                element.classList = 'preview';
        element.setAttribute('data-preview', id);
